defmodule EsTest do
  ...
  test "returns a 201 on query creation" do
    ...
    {status, response} = Funnel.Es.register("token", body)
    {:ok, body} = JSEX.decode response
    assert status == 201
    Funnel.Es.unregister("token", body["filter_id"])
  end
  ...
  test "update a query creation" do
    ...
    {status, response} = Funnel.Es.register("token", uuid, body)
    {:ok, body} = JSEX.decode response
    assert status == 200
    Funnel.Es.unregister("token", body["filter_id"])
  end
  ...
  test "returns a body on query creation" do
    body = '{"query" : {"term" : {"field1" : "value1"}}}'
    {_, response} = Funnel.Es.register("token", body)
    {:ok, body} = JSEX.decode response
